...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, bool show_name, float combo_length)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float w = (flags & ImGuiComboFlags_NoPreview) ? arrow_size : show_name ? CalcItemWidth() + label_size.x : combo_length;//remove label display length
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y*2.0f));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? show_name ? style.ItemInnerSpacing.x + label_size.x : 0.0f : 0.0f, 0.0f));
    ItemSize(total_bb, style.FramePadding.y);
    if (!ItemAdd(total_bb, id, &frame_bb))
        return false;
...
bool ImGui::ListBoxHeader(const char* label, const ImVec2& size_arg, bool show_name)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImVec2 size = CalcItemSize(size_arg, show_name ? CalcItemWidth() : 0.0f, GetTextLineHeightWithSpacing() * 7.4f + style.ItemSpacing.y);
    ImVec2 frame_size = ImVec2(size.x, ImMax(size.y, label_size.y));
    ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + frame_size);
    ImRect bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? show_name ? style.ItemInnerSpacing.x + label_size.x: 0.0f : 0.0f, 0.0f));
...
bool ImGui::ListBoxHeader(const char* label, int items_count, int height_in_items, bool show_name)
{
    // Size default to hold ~7 items. Fractional number of items helps seeing that we can scroll down/up without looking at scrollbar.
    ...
    return ListBoxHeader(label, size,show_name);
}
...
bool ImGui::ListBox(const char* label, int* current_item, const char* const items[], int items_count, int height_items, bool show_name)
{
    const bool value_changed = ListBox(label, current_item, Items_ArrayGetter, (void*)items, items_count, height_items, show_name);
    return value_changed;
}

bool ImGui::ListBox(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int height_in_items, bool show_name)
{
    if (!ListBoxHeader(label, items_count, height_in_items,show_name))
        return false;
...
void ImGui::Columns(int columns_count, const char* id, bool border, ImGuiColumnsFlags flags)
{
    ...
    flags = flags | (border ? 0 : ImGuiColumnsFlags_NoBorder);
